...
export const ActiveEnvironment = Context.Reference<ApplicationEnvironment>("ActiveEnvironment", {
...
export class CurrentApplication extends Context.Service<
  CurrentApplication,
...
>()("ApplicationAuthentication", {
  ...
  security: {
    appKeyHeader: HttpApiSecurity.apiKey({
      key: "x-app-key",
      in: "header",
    }).pipe(HttpApiSecurity.annotate(OpenApi.Description, "Application key header")),
  },
}) {}
    ...
    return {
      appKeyHeader: (effect, opts) =>
        Effect.gen(function* () {
          ...
          const appContext = CurrentApplicationContext.make({
            id: "repro-app",
            name: "Repro App",
            environment: "live",
          });
